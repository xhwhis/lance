...
#[derive(Debug)]
pub struct ContainsUdf {
    signature: Signature,
    aliases: Vec<String>,
}

impl Default for ContainsUdf {
    fn default() -> Self {
        Self::new()
    }
}
                    ...
                    .map(|opt| match opt {
                        (Some(wkb1), Some(wkb2)) => {
                            match (Geometry::new_from_wkb(wkb1), Geometry::new_from_wkb(wkb2)) {
                                (Ok(geom1), Ok(geom2)) => geom1.contains(&geom2).ok(),
                                _ => None,
                            }
                        }
                        _ => None,
                                ...
                                .map(|opt| {
                                    opt.and_then(|wkb1| {
                                        Geometry::new_from_wkb(wkb1)
                                            .ok()
                                            .and_then(|geom1| geom1.contains(&geom2).ok())
                                    })
                                })
                                .collect();
                            Arc::new(result)
                                ...
                                .map(|opt| {
                                    opt.and_then(|wkb2| {
                                        Geometry::new_from_wkb(wkb2)
                                            .ok()
                                            .and_then(|geom2| geom1.contains(&geom2).ok())
                                    })
                                })
                                .collect();
                            Arc::new(result)
